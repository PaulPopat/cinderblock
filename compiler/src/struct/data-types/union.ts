...
export class Union<TOptions extends any[]> implements ISerialiseable<TOptions[number]> {
  ...
  Impart(value: TOptions[number], buffer: IBufferWriter): void {
    ...
    if (match_index === -1) throw new Error("No match found during union serialisation");
