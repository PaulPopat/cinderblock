...
export class Guid implements ISerialiseable<string> {
  ...
  Accept(buffer: IBufferReader): string {
    ...
    return [result.substring(0, 8), result.substring(8, 12), result.substring(12, 16), result.substring(16, 20), result.substring(20, 32)].join("-");
  }

  Confirm(value: unknown): value is string {
    return typeof value === "string" && !!value.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/gm);
  }
}
