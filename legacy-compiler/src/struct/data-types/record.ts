...
export class Record<TKey extends string | number, TValue> implements ISerialiseable<VanillaRecord<TKey, TValue>> {
  ...
  Confirm(value: unknown): value is VanillaRecord<TKey, TValue> {
    ...
    for (const key in value) if (!this.#key.Confirm(key) || !this.#value.Confirm((value as any)[key])) return false;
