...
type UnionToIntersection<T extends unknown[]> = T extends [infer F, ...infer R] ? F & UnionToIntersection<R> : unknown;

export class Intersection<TOptions extends any[]> implements ISerialiseable<UnionToIntersection<TOptions>> {
  ...
  Accept(buffer: IBufferReader): UnionToIntersection<TOptions> {
    ...
    for (const s of this.#structure) result = { ...result, ...s.Accept(buffer) };
